...
type ChoiceGoal = 'Best Route' | 'Trophy Cleanup' | 'Death Route' | 'Ending Route';

const goals: ChoiceGoal[] = ['Best Route', 'Trophy Cleanup', 'Death Route', 'Ending Route'];
...
const rows = [
  ...
  },
  {
    ...
    goal: 'Trophy Cleanup',
    impact: 'Best way to test deaths, endings, and collectibles one variable at a time.',
    status: 'Recommended',
  },
  {
  ...
  },
  {
    ...
    goal: 'Ending Route',
    impact: 'Fastest way to compare ending variants without replaying the whole story.',
    status: 'Recommended',
  },
];
...
export function ChoiceImpactFilter() {
  ...
  return (
    <section aria-labelledby="choice-impact-filter" className="my-10 rounded-lg border border-d8020/40 bg-card/40 p-5">
      ...
      <h2 id="choice-impact-filter" className="text-xl font-bold text-foreground">
      ...
      <div className="mt-4 grid gap-3">
        {visibleRows.map((row) => (
          <div key={row.choice} className="rounded-lg border border-border/50 bg-background/35 p-4">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <h3 className="text-base font-semibold text-foreground">{row.choice}</h3>
              <span className="rounded bg-muted px-2 py-0.5 text-xs text-muted-foreground">{row.status}</span>
            </div>
            <p className="text-sm leading-relaxed text-muted-foreground">{row.impact}</p>
          </div>
        ))}
      </div>
    </section>
