...
export function DeathSceneTracker() {
  ...
  return (
    <section aria-labelledby="death-scene-tracker" className="my-10 rounded-lg border border-d8020/40 bg-card/40 p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button
          type="button"
          onClick={() => setChecked([])}
          className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-border/60 px-3 text-sm text-muted-foreground hover:border-d8020/60 hover:text-foreground"
        >
      ...
      <div className="mt-5">
        ...
        <div className="h-3 overflow-hidden rounded-full bg-muted">
          <div className="h-full rounded-full bg-d8020" style={{ width: `${percent}%` }} />
        </div>
      </div>
      ...
      <div className="mt-4 grid max-h-[360px] gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
        {deathSlots.map((slot) => {
          ...
          return (
            <label key={slot.id} className={`rounded-md border p-3 ${done ? 'border-red-400/40 bg-red-400/10' : 'border-border/50 bg-background/35'}`}>
