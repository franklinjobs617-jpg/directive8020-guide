...
export function MyProgressPanel() {
  ...
  return (
    <section aria-labelledby="my-progress" className="my-10 rounded-lg border border-d8020/40 bg-card/40 p-5">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <span className="rounded-md border border-border/50 px-3 py-2 text-sm font-bold text-d8020">
          {percent}% overall
        </span>
      </div>

      <div className="mb-4 h-3 overflow-hidden rounded-full bg-muted">
        <div className="h-full rounded-full bg-d8020 transition-all duration-300" style={{ width: `${percent}%` }} />
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {progressItems.map((item) => {
          ...
          return (
            <Link key={item.key} href={item.href} className="rounded-lg border border-border/50 bg-background/35 p-4 transition-colors hover:border-d8020/50">
              <Icon className="mb-3 h-5 w-5 text-d8020" />
