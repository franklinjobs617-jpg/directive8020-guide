...
export function EnginefallGuidePage({ page }: { page: EnginefallPageData }) {
 ...
 <h2 className="mb-4 text-2xl font-medium text-foreground">{section.title}</h2>
 {section.image ? (
 <ArticleImage
 src={section.image}
 ...
 <tr key={`${first}-${second}`} className="border-b border-border last:border-0">
 <th scope="row" className="px-4 py-3 text-left font-medium text-foreground">{first}</th>
