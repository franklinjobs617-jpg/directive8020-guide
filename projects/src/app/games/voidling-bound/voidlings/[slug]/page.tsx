      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[360px_1fr]" aria-labelledby="core-data-heading">
          <figure className="overflow-hidden rounded-lg border border-border/50 bg-card/30">
            <div className="relative aspect-square bg-background/55">
              <Image
            ...
            <figcaption className="border-t border-border/40 px-4 py-3 text-xs leading-relaxed text-muted-foreground">
          ...
          <div className="overflow-hidden rounded-lg border border-border/50 bg-card/30">
            <div className="border-b border-border/50 p-4">
              <h2 id="core-data-heading" className="text-xl font-black text-foreground">
                Core Fields
              </h2>
            </div>
        ...
        <section className="mt-8" aria-labelledby="abilities-heading">
          <h2 id="abilities-heading" className="mb-3 text-2xl font-black text-foreground">
            Abilities
          </h2>
        ...
        <section className="mt-8 grid gap-5 lg:grid-cols-2">
          <div className="rounded-lg border border-border/50 bg-card/30 p-5">
            <h2 className="text-xl font-black text-foreground">Modules</h2>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {entry.modules.map((module) => (
                      <tr key={`${module.name}-${module.bonus}`} className="border-b border-border/25 last:border-0">
          ...
          <div className="rounded-lg border border-border/50 bg-card/30 p-5">
            <h2 className="text-xl font-black text-foreground">Status Effects</h2>
            {entry.statusEffects.length > 0 ? (
              <div className="mt-3 flex flex-wrap gap-2">
                {entry.statusEffects.map((status) => (
                  <span key={status} className="rounded border border-d8020/35 bg-d8020/10 px-3 py-1 text-xs font-semibold text-d8020">
                    {status}
                  </span>
                ))}
              </div>
        ...
        <section className="mt-8 grid gap-5 lg:grid-cols-2">
          <div className="rounded-lg border border-border/50 bg-card/30 p-5">
            <h2 className="text-xl font-black text-foreground">Evolution Context</h2>
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
          ...
          <div className="rounded-lg border border-border/50 bg-card/30 p-5">
            <h2 className="text-xl font-black text-foreground">Source Link</h2>
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              Check the original{' '}
              <Link href={entry.wikiUrl} target="_blank" rel="noreferrer" className="text-d8020 hover:underline">
                wiki section for {entry.name}
              </Link>{' '}
              when exact wording matters. Then return to the{' '}
              <Link href="/games/voidling-bound/database" className="text-d8020 hover:underline">
                database
              </Link>{' '}
              for filters or the{' '}
        ...
        <section className="mt-10">
          <h2 className="mb-4 text-2xl font-black text-foreground">Related Voidlings</h2>
